...
class BaseEvaluator(ABC):
	def __init__(self, dataset_name: str, user_chosen_device: Device) -> None:
		self.dataset_name: str = dataset_name
		self.user_chosen_device: Device = user_chosen_device

	@staticmethod
	def create_evaluator(dataset_name: str, is_gpu_run: bool) -> "BaseEvaluator":
		...
		if learning_type == LearningType.self_supervised:
			...
			representation_model, representation_model_training_mode = BaseEvaluator.load_representation_model(
				prior_representation_config
			)
			return LegacyEvaluator(
				dataset_name, user_chosen_device, representation_model, representation_model_training_mode
			)
	...
	def _decide_device(self) -> Device:
		if self.user_chosen_device == Device.CPU:
			...
			if torch.cuda.is_available() is False and torch.backends.mps.is_available() is False:
				logger.warning(
					f"User chose training in {self.user_chosen_device.name} but CUDA/MPS is not available. "
	...
	def _build_callbacks(
		self, model_saving_dir: str, metadata_info: MetadataInfo, train_time: float, early_stop: int | None
	) -> list[Callback]:
		callbacks: list[Callback] = [
			ModelCheckpointCallback(model_saving_dir, metadata_info),
		...
		if fitness_metric_name in [FitnessMetricName.DOWNSTREAM_ACCURACY, FitnessMetricName.KNN_ACCURACY]:
			...
			if fitness_metric_name != fitness_used:
				...
				dataset_copy.transform = LegacyTransformer(
					{"random_crop": {"size": 32, "padding": 4}, "random_horizontal_flip": {"probability": 0.5}}
				)
				kwargs["dataset"] = dataset_copy
			else:
				kwargs["dataset"] = dataset
			kwargs["dataset_name"] = self.dataset_name
			loaders_to_use[DatasetType.DOWNSTREAM_TRAIN] = DataLoader(
				dataset[DatasetType.DOWNSTREAM_TRAIN],
...
class LegacyEvaluator(BaseEvaluator):
	...
	def evaluate(
		...
		dataset_input_size: Size = Size(list(DATASETS_INFO[self.dataset_name]["expected_input_dimensions"]))
		input_size: Size
		...
		try:
			...
			if self.representation_model is not None:
				...
				required_grad: bool = (
					False if self.representation_model_training_mode == DownstreamMode.freeze else True
				)
			...
			learning_params: LearningParams = ModelBuilder.assemble_optimiser(training_parameters, optimiser)
			...
			metadata_info: MetadataInfo = MetadataInfo.new_instance(
				self.dataset_name, dataset, optimiser, learning_params, None
			)
			...
			trainer = Trainer(
				...
				callbacks=self._build_callbacks(
					model_saving_dir, metadata_info, train_time, learning_params.early_stop
				),
			...
			fitness_value = Fitness(
				fitness_metric.compute_metric(torch_model, loaders_dict, device), type(fitness_metric)
			)
...
class BarlowTwinsEvaluator(BaseEvaluator):
	...
	def evaluate(
		...
		model_builder: ModelBuilder = ModelBuilder(
			parsed_network, parsed_projector_network, device, Size(list(input_size))
		)
		...
		try:
			...
			metadata_info: MetadataInfo = MetadataInfo.new_instance(
				self.dataset_name, dataset, optimiser, learning_params, pretext_task
			)
			start_epoch: int = num_epochs if restart_train is False else 0
			trainer = Trainer(
				model=torch_model,
			...
			fitness_value: Fitness
			if (
				isinstance(fitness_metric, AccuracyMetric)
				or isinstance(fitness_metric, DownstreamAccuracyMetric)
				or isinstance(fitness_metric, KNNAccuracyMetric)
			):
				fitness_value = Fitness(
					fitness_metric.compute_metric(torch_model, loaders_dict, device), type(fitness_metric)
				)
			...
			logger.warning(
				f"Invalid model, error during evaluation. Fitness will be computed as invalid individual."
